...
figure; plot(t); %plot
xlabel('n'); ylabel('nTs (s)'); title('Discrete time values');
grid on;
pause;
%----------------- discrete time signal generation (A-D
conversion)--------------------
s1 = a1*sin(2*pi*f1*t); %generate sinusoid (double precision floating point)
figure; plot(t,s1,'-o'); %plot
xlabel('t'); ylabel('s1'); title('Discrete time sinusoid (f1)');
grid on
pause;
s1ADC = floor(s1*(2^(ADCBits-1))); %quantise to ADC scale (assuming ADC input
range is +/-1)
figure; plot(t,s1ADC,'-o'); %plot
xlabel('t'); ylabel('ADC value'); title('Quantised discrete time sinusoid (ADC values)');
grid on
pause;
...
xlabel('t'); ylabel('Voltage'); title('Quantised discrete time sinusoid (DAC voltage)');
grid on
hold; plot(t,Error,'r')
legend('Signal','Error')
pause;
