...
Sig = sin(2*pi*f*t); %generate sinusoid (double precision floating
point)
Channel = [2.5 5 1]; %interference channel coefficients (the unknown
coupling function)
...
Noise = randn(1,length(Sig)); %generate white noise signal (unknown additive
noise)
y = 1.0*Sig + Interference(1:length(Sig)) + 0.1*Noise; % signal + interference +
random noise
...
for i=1:length(y)
for n=N:-1:2
x(n) = x(n-1); %shift input delay line to the right by 1 sample (last
sample is forgotten)
end
x(1) = Reference(i); %insert most recent reference sample at start of
delay line
