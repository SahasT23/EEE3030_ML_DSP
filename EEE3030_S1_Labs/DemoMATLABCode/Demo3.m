...
x=randn(1,N); % generate normally distributed, random sequence with variance
of 1
y=randn(1,N); % generate another normally distributed, random sequence with
variance of 1
...
figure; plot(y);
Cxy = conv(fliplr(x),y); %cross correlate (matched filter is
convolution with time reversed signal)
figure;
plot(Cxy)
xlabel('lag (samples)');
ylabel('Correlation');
