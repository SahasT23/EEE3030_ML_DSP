...
The code reads the audio file and converts it to a row vector, handling stereo channels by selecting the first channel to ensure consistent downstream processing. Spectrogram analysis has been added as well, which will be shown below.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{T1P1TermAudioInfo.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{T1P1FreqDomTermOutput.png}
    \caption{AM band identification}
    \label{fig:enter-label}
\end{figure}

The spectrogram shows strong energy bands around $15--17 kHz$, with three distinct bright horizontal regions indicating short-duration high-frequency tonal bursts, representing the three characters. 
...
\textbf{Figure X SPECTRO} shows the spectrogram clearly:
...
Further terminal output prints out band verification.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{T1P2AMBandID.png}
    \caption{AM band identification}
    \label{fig:enter-label}
\end{figure}
...
Referring to \textbf{Figure X}, the Hamming window output shows a very slight reduction in noise floor compared to the unwindowed spectrum, clarifying the AM signal boundaries.
...
In addition, looking at the magnitude response shows that the filter follows the requirements.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{T3ComparisonSpectrumBPFilteredSig.png}
    \caption{Before and after signal squaring in the frequency domain}
    \label{fig:enter-label}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.35]{T3CarrierRecovVerifyTerm.png}
    \caption{AM band identification}
    \label{fig:enter-label}
\end{figure}
...
The code generates a cosine wave (looks like a green block as it has been zoomed out) at the recovered carrier frequency with initial phase $\phi = 0$ (to be optimised in Task 5). Element-wise multiplication performs the mixing operation, followed by Hamming-windowed FFT analysis to quantify power in the baseband (0--4 kHz) and $2f_c$ regions.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.17]{T4SNRAndPowerTermOutput.png}
...
\begin{enumerate}
  \item Plays the audio clip after applying all of the prior filtering etc.
  \item Tests and plots 3 different phase optimisation techniques and compares theoretical output with practical.
  \item Terminal output showing the SNR comparisons for each of the three methods. 
  \item A plot output showing phase optimisation effects on a small section of the audio signal, demodulated output.
  \item Plot with signal and envelope overlap. 
\end{enumerate}

\textbf{Results and Discussion}

From \textbf{Figure X}, three optimisation techniques have been used (technically 2). The first is coarse grid search, finding the amplitudes in $5^\circ$ steps and comparing for the maximum amplitude.
Fine grid search does the same, in $1^\circ$ steps instead. Golden section [source] is an efficient optimisation algorithm that reduces the search interval by a factor of $1/\varphi$ (the golden ratio) at each iteration. It converges in $\mathcal{O}\!\left(\log\!\left(\tfrac{1}{\varepsilon}\right)\right)$ iterations, where $\varepsilon$ is the desired tolerance, a slight improvement in speed.

It is easily visible that the $120^\circ$ or $2.0944\mathrm{rad}$ is the phase wherein the maximum amplitude can be achieved (using an RMS amplitude score/metric - the highest being $0.063828$), with all three plots verifying this phase. 

\begin{large}
FIGURE SHOWING THE 3 OPTIMISATION METHODS VISUAL OUTPUTs
\end{large}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.17]{T5PhaseOptPlots.png}
    \caption{3 phase optimisation plots}
    \label{fig:enter-label}
\end{figure}

For further verification, \textbf{Figure X} below shows the terminal output for the optimisation calculation.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.17]{T5PhaseOptTechniqTerm.png}
    \caption{Terminal output showing the optimisation results}
    \label{fig:enter-label}
\end{figure}

SNR analysis can also verify the output signal and quantify how clear the audio is as well, the follwing metrics and their importance shown below, in \textit{Table X}:

\begin{table}[h!]
\centering
\caption{Summary of SNR Estimation Methods and Their Suitable Use Cases}
\begin{tabular}{l l l}
\hline
\textbf{Method} & \textbf{Principle} & \textbf{Best For} \\
\hline
Frequency Bands & Power ratio between speech and noise bands & Clear separation of signal/noise in frequency \\
Peaks vs Floor  & Signal peaks above noise floor              & Tonal signals with distinct peaks \\
Time Envelope   & Speech activity detection                   & Signals with varying amplitude over time \\
\hline
\end{tabular}
\end{table}

\textbf{Figure X} below shows the terminal output for the SNR metric calculations:

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.17]{T5SNRMeasTerm.png}
    \caption{}
    \label{fig:enter-label}
\end{figure}

Four methods have been implemented, as it can be seen from above. The first method looks at the speech band and compares the power to the noise band, which has been highlighted, resulting in $9.94dB$.

Method 2 wasn't particularly useful, comparing the peaks and the noise floor, reuslting in a meagre $2.77dB$, not particularly useful.

Phase optimisation only performed slightly better than Method 2, with a $3.64dB$ SNR value. 

Time domain envelope analysis performed the best, with $16.89dB$. 

For the final signal, phase optimisation did improve the audio quality marginally, as can be seen from \textbf{Figure From T4 with the time series signal before and after filtering with LP}, with an amplitude improvement of $1.52 \times$.

The final output was ``KVH''. The final, demodulated signal looks like this in the time domain:

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.17]{T5FinalTDSig.png}
    \caption{Final Time Domain Audio Signal}
    \label{fig:enter-label}
\end{figure}
